...
if [[ -z "$SRC_BOOTSTRAP_SERVERS"  ]] || [[ -z "$CCLOUD_BOOTSTRAP_SERVERS"  ]] || [[ -z "$CCLOUD_COMMAND_CONFIG"  ]]
then
    echo "--src-bootstrap-server, --ccloud-bootstrap-server, and --command-config are required for execution."
    show_usage
    exit 1
fi
...
then
    ...
    then
        ...
        do
            ...
            then
                ...
                kafka-consumer-groups --bootstrap-server ${SRC_BOOTSTRAP_SERVERS} --group ${group_name} --describe > ${SRC_GROUP_DATA}
    ...
    then
        ...
        # Describe the Consumer Groups in SRC and DEST
        kafka-consumer-groups --bootstrap-server ${SRC_BOOTSTRAP_SERVERS} --all-groups --describe > ${SRC_GROUP_DATA}
        kafka-consumer-groups --bootstrap-server ${CCLOUD_BOOTSTRAP_SERVERS} --command-config ${CCLOUD_COMMAND_CONFIG} --all-groups --describe > ${CCLOUD_GROUP_DATA}
